...
import com.wzu.lgw.appinfosystem.pojo.*;
import com.wzu.lgw.appinfosystem.service.*;
...
System.out.println(querySoftwareName+_queryStatus+_queryCategoryLevel1+_queryCategoryLevel2+_queryCategoryLevel3+_queryFlatformId);
        Integer devId = ((DevUser)session.getAttribute(Constants.devuserinfo)).getId();
        ...
        System.out.println(categoryLevel1List);
...
//            logger.info("uploadFile path: " + path);
        ...
        appInfo.setCreatedBy(((DevUser)session.getAttribute(Constants.devuserinfo)).getId());
        appInfo.setCreationDate(new Date());
        appInfo.setLogoPicPath(logoPicPath);
        appInfo.setLogoLocPath(logoLocPath);
        appInfo.setDevId(((DevUser)session.getAttribute(Constants.devuserinfo)).getId());
        appInfo.setStatus(1);
        try {
...
//            logger.info("uploadFile path: " + path);
            ...
            if(prefix.equalsIgnoreCase("apk")){//apk文件命名：apk名称+版本号+.apk
                ...
                if(apkName == null || "".equals(apkName)){
                    return "redirect:/devapp/appversionadd?id="+appVersion.getAppId()
                            +"&error=error1";
                }
        ...
        appVersion.setCreatedBy(((DevUser)session.getAttribute(Constants.devuserinfo)).getId());
    ...
    public Object sale(@PathVariable String appid,HttpSession session){
        ...
        if(appIdInteger>0){
            try {
                DevUser devUser = (DevUser)session.getAttribute(Constants.devuserinfo);
...
//            logger.info("uploadFile path: " + path);
        ...
        appVersion.setModifyBy(((DevUser)session.getAttribute(Constants.devuserinfo)).getId());
...
//            logger.info("uploadFile path: " + path);
            ...
            int filesize = 500000;
            if(attach.getSize() > filesize){//上传大小不得超过 50k
                return "redirect:/devapp/appinfomodify?id="+appInfo.getId()
                        +"&error=error4";
            }else if(prefix.equalsIgnoreCase("jpg") || prefix.equalsIgnoreCase("png")
                    ||prefix.equalsIgnoreCase("jepg") || prefix.equalsIgnoreCase("pneg")){//上传图片格式
        ...
        appInfo.setModifyBy(((DevUser)session.getAttribute(Constants.devuserinfo)).getId());
